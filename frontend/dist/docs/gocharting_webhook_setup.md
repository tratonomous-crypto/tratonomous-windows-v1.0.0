...
GoCharting is a professional HTML5 charting platform optimized for Indian markets. With a **Premium Plan**, you can set up webhook alerts to automate your trading strategies with Tratonomous.
...
### 2. Tratonomous Setup
- Tratonomous instance running and accessible
- API key configured in Tratonomous
...
If you're running Tratonomous locally (on your computer) and want to test webhooks, you'll need to expose your local server to the internet using a tunneling service.
...
2. Start the tunnel pointing to port 5000 (Tratonomous's default port)
3. Copy the public URL provided by the tunneling service
4. Update your `.env` file:
   ```env
   HOST_SERVER=https://your-tunnel-url.com
   ```
5. Restart Tratonomous application
...
1. **Login to Tratonomous**
   - Navigate to your Tratonomous dashboard
...
5. **Configure Delivery Methods**
   ...
   - ☑️ **Webhook**: Enable this for Tratonomous integration

   **Webhook URL**: Paste your Tratonomous webhook URL
   ```
   https://yourdomain.com/api/v1/placeorder
   ```
...
1. **Enable Sandbox Mode in Tratonomous**
...
1. Enable **Analyze Mode** in Tratonomous (toggle in navbar)
2. Create a test alert in GoCharting with a condition that will trigger immediately
3. Check the **API Analyzer** in Tratonomous to see the incoming request
...
```json
{
  "apikey": "your_api_key_here",      // Your Tratonomous API key
...
|-------|----------|-------------|--------------|
| apikey | Yes | Your Tratonomous API key | String |
...
4. Ensure Tratonomous is running and accessible from internet

### Issue: Orders not placing

**Solutions:**
1. Verify API key is correct in the JSON payload
2. Check broker connection in Tratonomous dashboard
3. Verify sufficient funds in trading account
4. Check symbol format matches your broker's convention
5. Review logs in Tratonomous (Logs menu)

### Issue: "API key not found" error

**Solutions:**
1. Go to Tratonomous → API Key section
...
4. **Test tunnel accessibility**
   ...
   - You should see Tratonomous login page
...
### 3. Monitor Activity
- Regularly check order logs in Tratonomous
...
Tratonomous has rate limiting to prevent abuse:
...
### Documentation
- Tratonomous Docs: https://docs.openalgo.in
...
A: Check the Logs section in Tratonomous or use the API Analyzer in Analyze Mode.
...
**Happy Trading with GoCharting and Tratonomous! 🚀**
